...
# === Subscription & Checkout (Stripe) additions ===
import os
...
STRIPE_SUCCESS_URL = os.getenv("STRIPE_SUCCESS_URL", "http://127.0.0.1:5000/signup?status=success")
STRIPE_CANCEL_URL = os.getenv("STRIPE_CANCEL_URL", "http://127.0.0.1:5000/signup?status=cancel")

# Individual tier price IDs (optional: use if you've created them in Stripe)
PRICE_ID_MONTHLY_INDIVIDUAL = os.getenv("PRICE_ID_MONTHLY_INDIVIDUAL", "").strip()
PRICE_ID_YEARLY_INDIVIDUAL = os.getenv("PRICE_ID_YEARLY_INDIVIDUAL", "").strip()
PRICE_ID_LIFETIME_INDIVIDUAL = os.getenv("PRICE_ID_LIFETIME_INDIVIDUAL", "").strip()

# Professional tier price IDs (optional)
PRICE_ID_MONTHLY_PRO = os.getenv("PRICE_ID_MONTHLY_PRO", "").strip()
PRICE_ID_YEARLY_PRO = os.getenv("PRICE_ID_YEARLY_PRO", "").strip()
PRICE_ID_LIFETIME_PRO = os.getenv("PRICE_ID_LIFETIME_PRO", "").strip()

# Fallback base pricing (in cents) used when a price ID is not provided.
BASE_AMOUNT_MONTHLY_CENTS = int(os.getenv("BASE_AMOUNT_MONTHLY_CENTS", "7900"))
BASE_AMOUNT_YEARLY_CENTS = int(os.getenv("BASE_AMOUNT_YEARLY_CENTS", "89900"))
BASE_AMOUNT_LIFETIME_CENTS = int(os.getenv("BASE_AMOUNT_LIFETIME_CENTS", "799900"))
PRO_MULTIPLIER = int(os.getenv("PRO_MULTIPLIER", "10"))
...
def _choose_price(plan: str, tier: str):
    """Return either a concrete price ID or a price_data dict for Checkout line item.
    tier: 'individual' | 'professional'
    plan: 'monthly' | 'yearly' | 'lifetime'
    """
    tier = (tier or 'individual').lower()
    plan = (plan or '').lower()

    # 1) If explicit price IDs are configured, use them
    by_tier = {
        'individual': {
            'monthly': PRICE_ID_MONTHLY_INDIVIDUAL,
            'yearly': PRICE_ID_YEARLY_INDIVIDUAL,
            'lifetime': PRICE_ID_LIFETIME_INDIVIDUAL,
        },
        'professional': {
            'monthly': PRICE_ID_MONTHLY_PRO,
            'yearly': PRICE_ID_YEARLY_PRO,
            'lifetime': PRICE_ID_LIFETIME_PRO,
        }
    }
    pid = by_tier.get(tier, {}).get(plan, "")
    if pid:
        return {'price': pid}

    # 2) Fallback to dynamic price_data based on base amounts
    mult = PRO_MULTIPLIER if tier == 'professional' else 1
    if plan == 'monthly':
        unit_amount = BASE_AMOUNT_MONTHLY_CENTS * mult
        return {'price_data': {
            'currency': CURRENCY,
            'recurring': {'interval': 'month'},
            'product_data': {'name': f'Signal Hawk {tier.title()} Monthly'},
            'unit_amount': unit_amount
        }}
    elif plan == 'yearly':
        unit_amount = BASE_AMOUNT_YEARLY_CENTS * mult
        return {'price_data': {
            'currency': CURRENCY,
            'recurring': {'interval': 'year'},
            'product_data': {'name': f'Signal Hawk {tier.title()} Yearly'},
            'unit_amount': unit_amount
        }}
    elif plan == 'lifetime':
        unit_amount = BASE_AMOUNT_LIFETIME_CENTS * mult
        return {'price_data': {
            'currency': CURRENCY,
            'product_data': {'name': f'Signal Hawk {tier.title()} Lifetime'},
            'unit_amount': unit_amount
        }}
    else:
        return None
...
def create_checkout_session():
    ...
    if plan not in {'monthly', 'yearly', 'lifetime'}:
        return ('Unknown plan. Expected one of: monthly, yearly, lifetime.', 400)

    st = _stripe_client()
    if st is None:
        return ('Stripe is not configured. Set STRIPE_API_KEY in your environment.', 500)

    item = _choose_price(plan, investor_type)
    if not item:
        return ('Unable to determine pricing for your selected plan/tier.', 400)

    try:
        # Determine mode by plan
        ...
        return (f'Failed to create Checkout Session: {e}', 500)

# Minimal account route to satisfy navbar links if not present elsewhere
@app.route('/account', methods=['GET'])
def account():
    return "<h2>Account</h2><p>Coming soon: manage your subscription here.</p>"
